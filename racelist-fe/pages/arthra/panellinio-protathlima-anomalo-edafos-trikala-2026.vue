<template>
  <div class="blog-content max-w-5xl mx-auto px-4 py-8">
    <h1 class="text-3xl text-center text-[#0057A0] my-6 font-bold">
      Πανελλήνιο Πρωτάθλημα σε ανώμαλο έδαφος - Τρίκαλα 2026
    </h1>

    <div class="prose prose-lg max-w-none">
      <div class="bg-blue-50 p-6 rounded-lg mb-8">
        <h2 class="text-2xl font-bold text-[#0057A0] mt-0 mb-4">
          Πληροφορίες Αγώνα
        </h2>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <p class="mb-2"><strong>Ημερομηνία:</strong> 1 Φεβρουαρίου 2026</p>
            <p class="mb-2"><strong>Τοποθεσία:</strong> Πηγή Τρικάλων</p>
            <p class="mb-2"><strong>Ώρα Έναρξης:</strong> 10:00 πμ</p>
          </div>
          <div>
            <p class="mb-2"><strong>Διοργάνωση:</strong> ΣΕΓΑΣ (Σ.Ε.Γ.Α.Σ.)</p>
            <p class="mb-2"><strong>Τύπος:</strong> Cross Country</p>
          </div>
        </div>
      </div>

      <h2 class="text-2xl font-bold text-[#0057A0] mt-8 mb-4">
        Πρόγραμμα Αγώνων
      </h2>

      <div class="overflow-x-auto mb-8">
        <table class="min-w-full bg-white border border-gray-300">
          <thead class="bg-[#0057A0] text-white">
            <tr>
              <th class="py-3 px-4 text-left">Ώρα</th>
              <th class="py-3 px-4 text-left">Αγώνισμα</th>
              <th class="py-3 px-4 text-left">Απόσταση</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-gray-200">
            <tr class="hover:bg-gray-50">
              <td class="py-3 px-4">10:00</td>
              <td class="py-3 px-4">Γυναικών Κ20</td>
              <td class="py-3 px-4">4χλμ.</td>
            </tr>
            <tr class="hover:bg-gray-50">
              <td class="py-3 px-4">10:20</td>
              <td class="py-3 px-4">Γυναικών Κ18</td>
              <td class="py-3 px-4">3χλμ.</td>
            </tr>
            <tr class="hover:bg-gray-50">
              <td class="py-3 px-4">10:35</td>
              <td class="py-3 px-4">Αγοριών Κ16</td>
              <td class="py-3 px-4">3χλμ.</td>
            </tr>
            <tr class="hover:bg-gray-50">
              <td class="py-3 px-4">10:55</td>
              <td class="py-3 px-4">Κοριτσιών Κ16</td>
              <td class="py-3 px-4">2χλμ.</td>
            </tr>
            <tr class="hover:bg-gray-50">
              <td class="py-3 px-4">11:15</td>
              <td class="py-3 px-4">Ανδρών Κ18</td>
              <td class="py-3 px-4">4χλμ.</td>
            </tr>
            <tr class="hover:bg-gray-50">
              <td class="py-3 px-4">11:35</td>
              <td class="py-3 px-4">Γυναικών και Γυναικών Κ23</td>
              <td class="py-3 px-4">8χλμ. / 6χλμ.</td>
            </tr>
            <tr class="hover:bg-gray-50">
              <td class="py-3 px-4">12:15</td>
              <td class="py-3 px-4">Ανδρών Κ20</td>
              <td class="py-3 px-4">6χλμ.</td>
            </tr>
            <tr class="hover:bg-gray-50">
              <td class="py-3 px-4">12:45</td>
              <td class="py-3 px-4">Ανδρών και Ανδρών Κ23</td>
              <td class="py-3 px-4">10χλμ. / 8χλμ.</td>
            </tr>
          </tbody>
        </table>
      </div>

      <h2 class="text-2xl font-bold text-[#0057A0] mt-8 mb-4">
        Αποτελέσματα Αγώνων
      </h2>

      <p class="text-lg mb-6">
        Το Πανελλήνιο Πρωτάθλημα σε ανώμαλο έδαφος στην Πηγή Τρικάλων
        πραγματοποιήθηκε με μεγάλη επιτυχία, συγκεντρώνοντας κορυφαίους αθλητές
        από όλη την Ελλάδα. Ο αγώνας διεξήχθη σε απαιτητική διαδρομή που έθεσε
        δοκιμασία την αντοχή και την ταχύτητα των συμμετεχόντων.
      </p>

      <div class="bg-yellow-50 p-6 rounded-lg mb-8">
        <h3 class="text-xl font-semibold mb-4 flex items-center gap-2">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            class="h-6 w-6"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
          >
            <path
              stroke-linecap="round"
              stroke-linejoin="round"
              stroke-width="2"
              d="M13 10V3L4 14h7v7l9-11h-7z"
            />
          </svg>
          Live Αποτελέσματα & Πληροφορίες
        </h3>
        <div class="space-y-3">
          <a
            href="https://my.raceresult.com/381222/results"
            target="_blank"
            rel="noopener noreferrer"
            class="block p-4 bg-white rounded-md hover:bg-gray-50 transition-colors border border-gray-200"
          >
            <div class="font-semibold text-[#0057A0]">
              📊 Αναλυτικά Αποτελέσματα
            </div>
            <div class="text-sm text-gray-600 mt-1">
              Δείτε τα πλήρη αποτελέσματα όλων των κατηγοριών στο Race Result
            </div>
          </a>

          <a
            href="https://meets.rosterathletics.com/public/competitions/details/schedule?id=28135"
            target="_blank"
            rel="noopener noreferrer"
            class="block p-4 bg-white rounded-md hover:bg-gray-50 transition-colors border border-gray-200"
          >
            <div class="font-semibold text-[#0057A0]">📋 Roster Athletics</div>
            <div class="text-sm text-gray-600 mt-1">
              Επίσημα αποτελέσματα και χρονομετρήσεις
            </div>
          </a>

          <a
            href="https://www.youtube.com/watch?v=lutoSelQKfM"
            target="_blank"
            rel="noopener noreferrer"
            class="block p-4 bg-white rounded-md hover:bg-gray-50 transition-colors border border-gray-200"
          >
            <div class="font-semibold text-[#0057A0]">📺 Live Streaming</div>
            <div class="text-sm text-gray-600 mt-1">
              Παρακολουθήστε το βίντεο των αγώνων στο YouTube
            </div>
          </a>

          <a
            href="https://www.segas.gr/events/panellinio-protathlima-se-anomalo-edafos-pigi-trikala/"
            target="_blank"
            rel="noopener noreferrer"
            class="block p-4 bg-white rounded-md hover:bg-gray-50 transition-colors border border-gray-200"
          >
            <div class="font-semibold text-[#0057A0]">
              🏃 Σελίδα Αγώνα ΣΕΓΑΣ
            </div>
            <div class="text-sm text-gray-600 mt-1">
              Επίσημη σελίδα του αγώνα στην ιστοσελίδα του ΣΕΓΑΣ
            </div>
          </a>
        </div>
      </div>

      <h2 class="text-2xl font-bold text-[#0057A0] mt-8 mb-4">
        Σημαντικές Πληροφορίες
      </h2>

      <div class="bg-gray-50 p-6 rounded-lg mb-8">
        <h3 class="text-xl font-semibold mb-4">Σχετικά Έγγραφα</h3>
        <ul class="space-y-2">
          <li class="flex items-start gap-2">
            <span class="text-[#0057A0]">📄</span>
            <span>Προκήρυξη Αγώνα</span>
          </li>
          <li class="flex items-start gap-2">
            <span class="text-[#0057A0]">📊</span>
            <span>Λίστα Συμμετοχών (Startlist)</span>
          </li>
          <li class="flex items-start gap-2">
            <span class="text-[#0057A0]">ℹ️</span>
            <span>Χρήσιμες Πληροφορίες και Επίσημη Ανακοίνωση</span>
          </li>
        </ul>
      </div>

      <h2 class="text-2xl font-bold text-[#0057A0] mt-8 mb-4">
        Στατιστικά Αγώνα
      </h2>

      <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        <div class="bg-gradient-to-br from-blue-50 to-blue-100 p-6 rounded-lg">
          <div class="text-3xl font-bold text-[#0057A0] mb-2">89</div>
          <div class="text-gray-700">Συνολικοί Συμμετέχοντες</div>
        </div>
        <div
          class="bg-gradient-to-br from-green-50 to-green-100 p-6 rounded-lg"
        >
          <div class="text-3xl font-bold text-[#0057A0] mb-2">8</div>
          <div class="text-gray-700">Κατηγορίες Αγώνων</div>
        </div>
        <div
          class="bg-gradient-to-br from-purple-50 to-purple-100 p-6 rounded-lg"
        >
          <div class="text-3xl font-bold text-[#0057A0] mb-2">Κ16-Ανδρών</div>
          <div class="text-gray-700">Εύρος Κατηγοριών</div>
        </div>
      </div>

      <h2 class="text-2xl font-bold text-[#0057A0] mt-8 mb-4">
        Σχετικά με τον Αγώνα
      </h2>

      <p class="text-lg mb-4">
        Το Πανελλήνιο Πρωτάθλημα σε ανώμαλο έδαφος αποτελεί έναν από τους πιο
        σημαντικούς αγώνες του ελληνικού στίβου. Ο αγώνας στην Πηγή Τρικάλων
        διοργανώνεται υπό την αιγίδα του ΣΕΓΑΣ (Σύνδεσμος Ελληνικών Αθλητικών
        Σωματείων) και συγκεντρώνει τους κορυφαίους αθλητές ανώμαλου εδάφους της
        χώρας.
      </p>

      <p class="text-lg mb-4">
        Το cross country running είναι ένας αγώνας δρόμου που διεξάγεται σε
        φυσικό έδαφος με ποικιλία υψομέτρου, άνηφορες, κατηφορικές και διάφορα
        εμπόδια. Απαιτεί εξαιρετική φυσική κατάσταση, δύναμη, αντοχή και
        τεχνική, καθιστώντας τον έναν από τους πιο απαιτητικούς αγώνες στον
        στίβο.
      </p>

      <div class="bg-blue-50 p-6 rounded-lg mt-8">
        <h3 class="text-xl font-semibold mb-4">Γιατί είναι Σημαντικός;</h3>
        <ul class="space-y-2">
          <li class="flex items-start gap-2">
            <span class="text-[#0057A0] font-bold">✓</span>
            <span
              >Καθορίζει τους Πανελλήνιους Πρωταθλητές σε όλες τις
              κατηγορίες</span
            >
          </li>
          <li class="flex items-start gap-2">
            <span class="text-[#0057A0] font-bold">✓</span>
            <span
              >Χρησιμεύει ως επιλεκτικός αγώνας για διεθνείς διοργανώσεις</span
            >
          </li>
          <li class="flex items-start gap-2">
            <span class="text-[#0057A0] font-bold">✓</span>
            <span
              >Προωθεί τον αθλητισμό και το cross country running στην
              Ελλάδα</span
            >
          </li>
          <li class="flex items-start gap-2">
            <span class="text-[#0057A0] font-bold">✓</span>
            <span
              >Δίνει την ευκαιρία σε νέους αθλητές να διακριθούν σε εθνικό
              επίπεδο</span
            >
          </li>
        </ul>
      </div>

      <div class="mt-8 p-6 bg-gray-50 rounded-lg">
        <h3 class="text-xl font-semibold mb-4">Επικοινωνία ΣΕΓΑΣ</h3>
        <div class="space-y-2 text-gray-700">
          <p><strong>Διεύθυνση:</strong> Λ. Συγγρού 137, Νέα Σμύρνη, 171 21</p>
          <p><strong>Πόλη:</strong> Αθήνα, Ελλάδα</p>
          <p><strong>Τηλέφωνο:</strong> 211 18 777 00-01</p>
          <p><strong>Website:</strong> www.segas.gr</p>
        </div>
      </div>

      <div class="mt-8 text-center text-gray-600">
        <p class="text-sm">Τελευταία ενημέρωση: 1 Φεβρουαρίου 2026</p>
      </div>
    </div>
  </div>
</template>

<script setup>
  useHead({
    title:
      'Πανελλήνιο Πρωτάθλημα σε ανώμαλο έδαφος - Τρίκαλα 2026 | Αποτελέσματα',
    link: [
      {
        rel: 'canonical',
        href: 'https://racelist.gr/arthra/panellinio-protathlima-anomalo-edafos-trikala-2026/',
      },
    ],
    meta: [
      {
        name: 'description',
        content:
          'Αποτελέσματα και πληροφορίες από το Πανελλήνιο Πρωτάθλημα σε ανώμαλο έδαφος (Cross Country) στην Πηγή Τρικάλων, 1 Φεβρουαρίου 2026. Δείτε τα αποτελέσματα όλων των κατηγοριών.',
      },
      {
        property: 'og:title',
        content:
          'Πανελλήνιο Πρωτάθλημα σε ανώμαλο έδαφος - Τρίκαλα 2026 | Αποτελέσματα',
      },
      {
        property: 'og:description',
        content:
          'Αποτελέσματα και πληροφορίες από το Πανελλήνιο Πρωτάθλημα σε ανώμαλο έδαφος (Cross Country) στην Πηγή Τρικάλων. 89 αθλητές, 8 κατηγορίες αγώνων.',
      },
      {
        property: 'og:type',
        content: 'article',
      },
      {
        name: 'keywords',
        content:
          'πανελληνιο πρωταθλημα, cross country, ανωμαλο εδαφος, τρικαλα, πηγη τρικαλων, segas, σεγας, αποτελεσματα, αγωνες, στιβος',
      },
    ],
  });

  useJsonld([
    {
      "@context": "https://schema.org",
      "@type": "Article",
      "headline": "Πανελλήνιο Πρωτάθλημα σε ανώμαλο έδαφος - Τρίκαλα 2026 | Αποτελέσματα",
      "description": "Αποτελέσματα και πληροφορίες από το Πανελλήνιο Πρωτάθλημα σε ανώμαλο έδαφος (Cross Country) στην Πηγή Τρικάλων, 1 Φεβρουαρίου 2026. Δείτε τα αποτελέσματα όλων των κατηγοριών.",
      "image": "https://racelist.gr/racelist-banner.jpg",
      "url": "https://racelist.gr/arthra/panellinio-protathlima-anomalo-edafos-trikala-2026/",
      "author": {
        "@type": "Organization",
        "name": "racelist.gr",
        "url": "https://racelist.gr"
      },
      "publisher": {
        "@type": "Organization",
        "name": "racelist.gr",
        "url": "https://racelist.gr"
      },
      "datePublished": "2026-02-01"
    },
    {
      "@context": "https://schema.org",
      "@type": "BreadcrumbList",
      "itemListElement": [
        {
          "@type": "ListItem",
          "position": 1,
          "name": "Αρχική",
          "item": "https://racelist.gr/"
        },
        {
          "@type": "ListItem",
          "position": 2,
          "name": "Άρθρα",
          "item": "https://racelist.gr/arthra/"
        },
        {
          "@type": "ListItem",
          "position": 3,
          "name": "Πανελλήνιο Πρωτάθλημα σε ανώμαλο έδαφος - Τρίκαλα 2026 | Αποτελέσματα",
          "item": "https://racelist.gr/arthra/panellinio-protathlima-anomalo-edafos-trikala-2026/"
        }
      ]
    },
  ]);

</script>

<style scoped>
  .blog-content {
    font-family:
      system-ui,
      -apple-system,
      sans-serif;
  }

  .prose {
    color: #374151;
    line-height: 1.75;
  }

  .prose h2 {
    margin-top: 2rem;
    margin-bottom: 1rem;
  }

  .prose h3 {
    margin-top: 1.5rem;
    margin-bottom: 0.75rem;
  }

  .prose p {
    margin-bottom: 1rem;
  }

  .prose ul {
    margin-bottom: 1rem;
  }

  .prose a {
    color: #0057a0;
    text-decoration: none;
  }

  .prose a:hover {
    text-decoration: underline;
  }

  table {
    border-collapse: collapse;
  }

  th {
    font-weight: 600;
  }
</style>
